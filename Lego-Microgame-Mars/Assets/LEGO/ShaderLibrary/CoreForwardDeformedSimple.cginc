#ifndef CORE_FORWARD_DEFORMED_SIMPLE_INCLUDED
#define CORE_FORWARD_DEFORMED_SIMPLE_INCLUDED
...
VertexOutputBaseSimple vertForwardBaseSimple (VertexInput v)
{
    ...
    // Do custom computation of positions.
    float4 posWorld = mul(deformMatrix, mul(unity_ObjectToWorld, v.vertex));
    o.pos = UnityWorldToClipPos(posWorld);
    o.tex = TexCoords(v);
    
    half3 eyeVec = normalize(posWorld.xyz - _WorldSpaceCameraPos);

    // Do custom computation of normal.
    half3 normalWorld = SafeNormalize(mul(UnityObjectToWorldNormal(v.normal), (float3x3)deformMatrix))

    // We do not handle normal maps properly!
...
#endif // CORE_FORWARD_DEFORMED_SIMPLE_INCLUDED